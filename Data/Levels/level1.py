...
import globe
from Managers.Class import bullet, item
from Data import orbit

import random
import time

from GUI import gameover
...
def init():
	...
	orb["p1e1"]= orbit.quickcreate(102, p1=(96, 0), p2=(128, 196), speed=2) #3
	orb["p1e1"].create_bullet=selfsniper

	orb["p1e2"]= orbit.quickcreate(102, p1=(288, 0), p2=(256, 196), speed=2) #3
	orb["p1e2"].create_bullet=selfsniper

	orb["p1e3"]= orbit.quickcreate(102, p1=(96, globe.game_active_rect.bottom), p2=(128, globe.game_active_rect.bottom - 196), speed=1) #1.5
	orb["p1e3"].create_bullet=roundrand

	orb["p1e4"]= orbit.quickcreate(102, p1=(288, globe.game_active_rect.bottom), p2=(256, globe.game_active_rect.bottom - 196), speed=1) #1.5
	orb["p1e4"].create_bullet=roundrand2

	orb["p2e1"]= orbit.quickcreate(101, point=(0, 0), speedx=0, speedy=2) #3
	orb["p2e1"].create_bullet=p2e1roundrand

	orb["p2e2"]= orbit.quickcreate(101, point=(0, 0), speedx=0, speedy=2) #3
	orb["p2e2"].create_bullet=p2e2roundrand

	orb["p3e1"]= orbit.quickcreate(103, p1=(0, 0), p2=(1, 1), speed=1.5, time=600) #2
	...
	orb["p5e1"]= orbit.quickcreate(103, p1=(0, 0), p2=(1, 1), speed=1.5, time=850) #2
...
def update(time):
	...
	if time>=60 and time<=240:
		...
		globe.mgame.call(gameover.GameOver)
