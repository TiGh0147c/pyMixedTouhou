# -*- coding: UTF-8 -*-
import pygame
import Managers.Media.resource  # 导入贴图资源
import globe				# 设定全局变量
import cache				# 导入贴图缓存
from Managers.Media import music
from GUI import launching
...
class Main_Window(object):
	...
	def __init__(self):
		...
		pygame.display.set_caption("东方大作业 ~ a Python Programming Project")
	...
	def init(self):
		...
		self.rsmanager = Managers.Media.resource.Resource()				# 定位贴图资源管理器
		self.msmanager = music.MusicManager()							# 定位音频管理器
		self.myfont = pygame.font.SysFont("Consolas", 15)	# 定义用于显示帧速率的字体
		self.goto(launching.Launching)									# 切换至加载界面 'init', 开始游戏
