...
class PauseMenu(object):
	...
	def __init__(self):
		...
		# 一层菜单
		self.button_rect.append([35, 320])			# 定位 'Resume_Start' 按键位置
		self.button_rect.append([40, 352])			# 定位 'To_Title_Start' 按键位置
		self.button_rect.append([40, 384])			# 定位 'Retry_Start' 按键位置
		# 二层菜单
		self.button_rect.append([80, 320])				# 定位二层菜单中 'Yes' 按键位置
		self.button_rect.append([80, 352])				# 定位二层菜单中 'No' 按键位置
	...
	def event_control(self):
		...
		if not globe.scene_menu_choose:
			...
			if self.index == 1:
				...
				globe.mgame.call(confirm.Confirm)
	...
	def draw(self, screen):
		"""定义菜单绘制函数"""
		# 绘制 Logo 和屏幕按键
		screen.blit(self.pause_title, (40, 288))
