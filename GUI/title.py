...
class TitleMenu(object):
	...
	def __init__(self):
		...
		self.logo = (
			pygame.image.load("./Resources/pic/title_logo.png").convert_alpha()).subsurface(0, 0, 200, 60)
		self.tbg = (
			pygame.image.load("./Resources/pic/tbg.png").convert_alpha()).subsurface(0, 0, 136, 220)
		self.tbg.set_alpha(75)
		self.rs = globe.mgame.rsmanager.image		# rs = resource

		self.button_rect.append([500, 125])			# 定位 'Game_Start' 按键位置
		self.button_rect.append([500, 160])			# 定位 'Extra_Start' 按键位置
		self.button_rect.append([500, 180])			# 定位 'Practice Start' 按键位置
		self.button_rect.append([500, 200])			# 定位 'Replay' 按键位置
		self.button_rect.append([500, 220])			# 定位 'Player Data' 按键位置
		self.button_rect.append([500, 240])			# 定位 'Music Room' 按键位置
		self.button_rect.append([500, 260])			# 定位 'Option' 按键位置
		self.button_rect.append([500, 280])			# 定位 'Quit' 按键位置
			...
			for event in pygame.event.get():
				...
				if event.type == KEYDOWN:
					...
					if event.key == K_UP:
						if self.index != 0:
							self.index -= 1
							self.button_rect[self.index + 1][1] += 15			# 按键微移动画
							globe.mgame.msmanager.play_SE("select")
					if event.key == K_DOWN:
						if self.index != 7:
							self.index += 1
							self.button_rect[self.index][1] -= 15				# 按键微移动画
							globe.mgame.msmanager.play_SE("select")
					if event.key == K_z:
						self.choose = True
						globe.mgame.msmanager.play_SE("select")
			...
			if self.flash >= 20:				# 按键闪烁 20 下后进行页面切换
				if self.index == 0:
					if self.flash >= 40:		# 按键闪烁 40 下后进行页面切换
						globe.mgame.goto(loading.Loading)
				if self.index == 1:
	...
	def draw(self, screen):
		"""定义菜单绘制函数"""
		# 绘制 Logo
		screen.blit(self.logo, (435, 415))
		screen.blit(self.tbg, (490, 100))
		# 绘制屏幕按键
		for i in range(0, 8):
